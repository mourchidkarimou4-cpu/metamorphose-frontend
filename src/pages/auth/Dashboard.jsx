...
function FormulaireTeomo({ user, onSuccess }) {
  ...
  async function submit(e) {
    ...
    try {
      const res = await fetch(API_URL + 'https://metamorphose-backend.onrender.com/api/avis/soumettre/', {
        method:"POST",
    ...
    async function load() {
      ...
      try {
        const [uRes, gRes, rRes, tRes] = await Promise.all([
          fetch(API_URL + 'https://metamorphose-backend.onrender.com/api/auth/me/',            { headers }),
          fetch(API_URL + 'https://metamorphose-backend.onrender.com/api/contenu/guides/',     { headers }),
          fetch(API_URL + 'https://metamorphose-backend.onrender.com/api/contenu/replays/',    { headers }),
          fetch(API_URL + 'https://metamorphose-backend.onrender.com/api/avis/mes-temoignages/', { headers }),
        ])
          ...
          <div style={{ maxWidth:'460px', animation:'fadeUp .5s both' }}>
            ...
            <div style={{ marginTop:'28px', display:'flex', gap:'12px', flexWrap:'wrap' }}>
                <button onClick={async () => {
                  const token = localStorage.getItem('mmorphose_token');
                  const res = await fetch(API_URL + 'https://metamorphose-backend.onrender.com/api/auth/certificat/', { headers:{'Authorization':`Bearer ${token}`} });
                  if(res.ok) {
