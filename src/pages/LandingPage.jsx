...
const STYLES = `
  ...
  *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
  ...
  /* ── Boutons ── */
  .btn-p{display:inline-flex;align-items:center;justify-content:center;gap:10px;background:var(--rose);color:#fff;font-family:var(--ff-b);font-weight:600;font-size:.76rem;letter-spacing:.16em;text-transform:uppercase;padding:16px 36px;border:none;border-radius:2px;cursor:pointer;text-decoration:none;transition:all .35s var(--ease);white-space:nowrap;min-width:260px}
  .btn-p:hover{background:#a01049;transform:translateY(-3px);box-shadow:0 14px 40px rgba(194,24,91,.4)}
  .btn-s{display:inline-flex;align-items:center;justify-content:center;gap:10px;background:transparent;color:var(--or);font-family:var(--ff-b);font-weight:500;font-size:.76rem;letter-spacing:.16em;text-transform:uppercase;padding:16px 36px;border:1px solid var(--or);border-radius:2px;cursor:pointer;text-decoration:none;transition:all .35s var(--ease);white-space:nowrap;min-width:260px}
      ...
      {menuOpen && (
        <div style={{ position:"fixed", inset:0, background:"#0A0A0A", zIndex:300, display:"flex", flexDirection:"column", overflowY:"auto" }}>
          ...
          <div style={{ padding:"20px 24px", borderTop:"1px solid rgba(201,169,106,.12)", display:"flex", flexDirection:"column", gap:"10px", flexShrink:0 }}>
            <button onClick={()=>{onRdvOpen();setMenuOpen(false);}}
  ...
  return (
    <section id="accueil" style={{ position:"relative", minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", textAlign:"center", padding:"100px 20px 100px", background:"var(--noir)", overflow:"hidden", color:"var(--blanc)", marginTop:"72px" }}>
      ...
      <div className="btn-group" style={{ position:"relative", display:"flex", flexWrap:"wrap", gap:"16px", justifyContent:"center", animation:"revealUp .9s .45s both", width:"100%", maxWidth:"480px" }}>
        <a href="#formules" className="btn-p" style={{ animation:"pulse-rose 3s ease-in-out infinite", width:"320px", justifyContent:"center" }}>
          {get("hero_btn1","Je rejoins l'aventure")}
        </a>
        <a href="#methode" className="btn-s" style={{ width:"320px", justifyContent:"center" }}>{get("hero_btn2","Découvrir le programme")}</a>
      </div>
...
function PartagerSection() {
  ...
  const reseaux = [
    { label:"WhatsApp", icon:"<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>", color:"#25D366", href:`https://wa.me/?text=${texte}%20${urlEnc}` },
    { label:"Facebook", icon:"<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>", color:"#1877F2", href:`https://www.facebook.com/sharer/sharer.php?u=${urlEnc}` },
    { label:"LinkedIn", icon:"<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>", color:"#0A66C2", href:`https://www.linkedin.com/sharing/share-offsite/?url=${urlEnc}` },
    { label:"X",        icon:"<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.741l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>",        color:"#000",    href:`https://twitter.com/intent/tweet?text=${texte}&url=${urlEnc}` },
  ];
  return (
    <div style={{ padding:"40px 24px", textAlign:"center", borderTop:"1px solid rgba(255,255,255,.04)" }}>
      ...
      <div style={{ display:"flex", gap:"10px", justifyContent:"center", flexWrap:"wrap" }}>
        {reseaux.map((r,i) => (
          <a key={i} href={r.href} target="_blank" rel="noreferrer" title={r.label}
            style={{ padding:"12px", background:r.color, color:"#fff", borderRadius:"6px", textDecoration:"none", display:"flex", alignItems:"center", justifyContent:"center" }}
...
function Ressources({ get }) {
  ...
  const [progress, setProgress] = useState(0);
  const ref = useRef(null);  // CORRECTION : vrai useRef pour que l'audio persiste entre rendus
      ...
      <div style={{ maxWidth:"900px", margin:"0 auto" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))", gap:"24px" }}>
          ...
          <div className="reveal" style={{ padding:"36px 32px", background:"rgba(194,24,91,.06)", border:"1px solid rgba(194,24,91,.15)", borderTop:"3px solid var(--rose)", borderRadius:"6px" }}>
            ...
            <audio ref={el => { ref.current = el; }} src={audioUrl} onTimeUpdate={onTimeUpdate} onEnded={() => { setPlaying(false); setProgress(0); }}/>
          ...
          <div className="reveal" style={{ padding:"36px 32px", background:"rgba(201,169,106,.04)", border:"1px solid rgba(201,169,106,.15)", borderTop:"3px solid var(--or)", borderRadius:"6px" }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:"8px", marginBottom:"24px" }}>
              {[get("res_guide_point1","Prioriser ce qui compte vraiment"), get("res_guide_point2","Sortir de la procrastination"), get("res_guide_point3","Passer à l'action avec discipline")].filter(Boolean).map((p,i) => (
            ...
            <a href={pdfUrl} download target="_blank" rel="noreferrer" style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:"10px", background:"transparent", color:"var(--or)", border:"1px solid var(--or)", borderRadius:"3px", padding:"13px 24px", fontFamily:"var(--ff-b)", fontWeight:600, fontSize:".72rem", letterSpacing:".12em", textTransform:"uppercase", textDecoration:"none", width:"100%", boxSizing:"border-box" }}>
        ...
        <div className="grid-footer" style={{ display:"grid", gridTemplateColumns:"1.5fr 1fr 1.2fr 1.2fr", gap:"32px", marginBottom:"48px" }}>
          ...
          <div>
            ...
            <div style={{ marginTop:"16px", display:"flex", flexWrap:"wrap", gap:"8px" }}>
              {[
                { label:"Instagram", icon:"<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg>", href:"https://www.instagram.com/preliaapedo?igsh=dGZhd2gxNnJjbXd3&utm_source=qr" },
                { label:"Facebook",  icon:"<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>",  href:"https://www.facebook.com/share/17mGLiwty7/?mibextid=wwXIfr" },
                { label:"TikTok",    icon:"<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z"/></svg>",    href:"https://www.tiktok.com/@prelitaa5?_r=1&_t=ZS-93NgdRO4zp6" },
                { label:"LinkedIn",  icon:"<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>",  href:"https://www.linkedin.com/in/pr%C3%A9lia-apedo-84572139b" },
              ].map((r,i) => (
                <a key={i} href={r.href} target="_blank" rel="noreferrer" title={r.label}
                  style={{ color:"rgba(201,169,106,.5)", textDecoration:"none", padding:"8px", border:"1px solid rgba(201,169,106,.15)", borderRadius:"4px", display:"flex", alignItems:"center", justifyContent:"center", transition:"all .2s" }}
                  onMouseEnter={e=>{ e.currentTarget.style.color="var(--or)"; e.currentTarget.style.borderColor="rgba(201,169,106,.4)"; }}
                  onMouseLeave={e=>{ e.currentTarget.style.color="rgba(201,169,106,.5)"; e.currentTarget.style.borderColor="rgba(201,169,106,.15)"; }}
                  dangerouslySetInnerHTML={{__html: r.icon}}/>
              ))}
            </div>
